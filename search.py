...
class Node:
    ...
    def expand(self, problem):
        ...
        #    cost =  problem.path_cost(self.path_cost, self.state, act, next)
        #print cost
...
def graph_my_search(problem, fringe, tipo):
    """Search through the successors of a problem to find a goal.
    The argument fringe should be an empty queue.
    If two paths reach a state, only use the best one. [Fig. 3.18]"""
    counter=0
...
def ramificacion_y_salto(problem, h=None):
    ...
    """
    return graph_my_search(problem, MYQueue(), 0)
